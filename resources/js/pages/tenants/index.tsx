...
export default function TenantsIndex({ table, title = 'Tenants', auth }: TenantsProps) {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedItems, setSelectedItems] = useState<number[]>([]);
    const [showDebug, setShowDebug] = useState(false);

    // Dados mock como fallback
    const mockData: TenantItem[] = [
        {
            id: 1,
    ...
    const items: TenantItem[] = table?.data || mockData;
    ...
    const handleExport = () => {
        router.visit(route('tenants.export'));
    };

    const handleView = (id: number) => {
        router.visit(route('tenants.show', id));
    };

    const handleEdit = (id: number) => {
        router.visit(route('tenants.edit', id));
    };
    ...
    const handleBulkDelete = () => {
        ...
        if (confirm(`Tem certeza que deseja excluir ${selectedItems.length} item(ns) selecionado(s)?`)) {
            router.post(route('tenants.bulk-delete'), {
                ids: selectedItems
            });
        }
    };
